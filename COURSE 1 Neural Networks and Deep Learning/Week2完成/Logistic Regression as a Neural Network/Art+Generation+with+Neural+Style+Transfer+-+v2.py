...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.reshape(a_C,(n_W*n_H,n_C))
    a_G_unrolled = tf.reshape(a_G,(n_W*n_H,n_C))
    
    # compute the cost with tensorflow (≈1 line)
    J_content = tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled,a_G_unrolled)))/(4.0*n_W*n_H*n_C)
    ### END CODE HERE ###
